...
export class DesigntocostStep4Component {
  ...
  ngOnInit(): void {
    ...
    this.CSHeaderId = '4483' // localStorage.getItem("DTCComapredId");
  ...
  newPDF() {
    ...
    const data = this.Vavedetails.map((val: { ID: any; Idea: any; PotentialSavingsPerPieceMDO: any; }) => [val.ID, val.Idea, val.PotentialSavingsPerPieceMDO]);
    ////  Vave Greade
    autoTable(doc, {
      head: headers,
      body: data,
  ...
  SaveVavedetails() {
    ...
    for (let i = 0; i < this.Vavedetails.length; i++) {
      vavelist.push({ CSHeaderId: this.CSHeaderId, SCReportId: this.SCReportId, IdeaId: this.Vavedetails[i].ID, PSavings: vavePercentText[i].value, CreatedBy: this.userId })
    }
