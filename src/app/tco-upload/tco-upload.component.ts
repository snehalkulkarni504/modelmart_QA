...
async gettcodetails() {
  debugger;
  const tcodata = await this.AdminService.GetTcoDetails().toPromise();
...
async submitForm() {
  ...
  try {
    const [filteredTcoDetails, filteredSupplierDetails] = await Promise.all([
      this.AdminService.GetFilteredTcoDetails(this.selectedRequestID, this.selectedModelMartID).toPromise(),
      this.AdminService.GetFilteredSupplierdetails(this.selectedRequestID, this.selectedModelMartID).toPromise()
    ]);
...
uploadFile(): void {
  if (this.selectedFiles.length > 0 && this.SupllierData.length == 0) {
    ...
    this.AdminService.uploadFile(this.selectedFiles[0], this.TCOID, this.TCO_Version,this.TCO_Number,this.selectedRequestID,this.selectedModelMartID,this.Status)
      .subscribe({
        next: (response) => {
    ...
    this.SupllierData.forEach(row => {
      ...
      this.AdminService.uploadFile(this.selectedFiles[0], row.TCOID, row.TCO_Number, row.TCO_Version,this.selectedRequestID,this.selectedModelMartID,this.Status)
        .subscribe({
          next: (response) => {
            console.log('File uploaded successfully:', response);
            alert('File uploaded successfully!');
...
//     this.AdminService.uploadFile(this.selectedFiles[0],this.TCO_ID,this.Tco_Number,this.Version)
//       .subscribe({
//         next: (response) => {
//           console.log('File uploaded successfully:', response);
//           alert('File uploaded successfully!');
...
//       this.AdminService.uploadFile(this.selectedFiles[0],row.TCO_ID,row.Tco_Number,row.Version)
//       .subscribe({
//         next: (response) => {
//           console.log('File uploaded successfully:', response);
//           alert('File uploaded successfully!');
...
//       this.AdminService.bulkUpload(this.selectedFiles).subscribe({
//         next: (response) => {
...
//       });
//     } else {
//       this.AdminService.uploadFile(this.selectedFiles[0],this.TCO_ID,this.Tco_Number,this.Version).subscribe({
//         next: (response) => {
//           console.log('File uploaded successfully:', response);
//           alert('File uploaded successfully!');
//           this.isUploading = true;
